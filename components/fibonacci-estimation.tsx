...
interface FibonacciEstimationProps {
  projectId: string;
  moduleVersion?: any;
...
export function FibonacciEstimation({ 
  projectId, 
  moduleVersion, 
  onEstimationUpdate 
}: FibonacciEstimationProps) {
  const [selectedLevel, setSelectedLevel] = useState<OptimisticLevel>('realistic');
  ...
  const loadEstimationPreview = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/projects/${projectId}/adjust-estimation?${params}`);
  ...
  const applyEstimation = async () => {
    ...
    setIsApplying(true);
    try {
      const response = await fetch(`/api/projects/${projectId}/adjust-estimation`, {
        method: 'POST',
        headers: {
  ...
  const getLevelColor = (level: OptimisticLevel) => {
    switch (level) {
      ...
      case 'realistic': return 'bg-blue-100 text-blue-800 border-blue-200';
      case 'pessimistic': return 'bg-orange-100 text-orange-800 border-orange-200';
    }
  };

  const getLevelDescription = (level: OptimisticLevel) => {
    switch (level) {
      case 'optimistic': return 'Best case scenario - ideal conditions, no blockers';
      case 'realistic': return 'Most likely scenario - normal development conditions';
      case 'pessimistic': return 'Worst case scenario - accounting for unknowns and challenges';
    }
  };
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Adjust your project estimation based on different scenarios using Fibonacci points.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {/* Team Velocity Setting */}
          <div className="space-y-2">
            <Label htmlFor="team-velocity">Team Velocity (points per sprint)</Label>
            <Input
              id="team-velocity"
              type="number"
              value={teamVelocity}
            ...
            <p className="text-sm text-muted-foreground">
              How many story points your team typically completes per sprint
            </p>
          </div>

          {/* Optimistic Level Selection */}
          <Tabs value={selectedLevel} onValueChange={(value) => setSelectedLevel(value as OptimisticLevel)}>
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="optimistic" className="text-green-700">Optimistic</TabsTrigger>
              <TabsTrigger value="realistic" className="text-blue-700">Realistic</TabsTrigger>
              <TabsTrigger value="pessimistic" className="text-orange-700">Pessimistic</TabsTrigger>
            </TabsList>
            
            <TabsContent value="optimistic" className="space-y-4">
              <div className={`p-4 rounded-lg border ${getLevelColor('optimistic')}`}>
            ...
            <TabsContent value="realistic" className="space-y-4">
              <div className={`p-4 rounded-lg border ${getLevelColor('realistic')}`}>
            ...
            <TabsContent value="pessimistic" className="space-y-4">
              <div className={`p-4 rounded-lg border ${getLevelColor('pessimistic')}`}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <Card>
                  <CardContent className="p-4">
                    <div className="text-2xl font-bold text-blue-600">
                      {estimation.totals.adjusted_points}
                    </div>
                    <p className="text-sm text-muted-foreground">Story Points</p>
                    {estimation.totals.original_points !== estimation.totals.adjusted_points && (
                      <p className="text-xs text-muted-foreground">
                        Original: {estimation.totals.original_points}
                      </p>
                    )}
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardContent className="p-4">
                    ...
                    {estimation.totals.original_hours !== estimation.totals.adjusted_hours && (
                      <p className="text-xs text-muted-foreground">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span>{Math.round((estimation.totals.adjusted_points / (estimation.totals.adjusted_points + 10)) * 100)}% complexity</span>
                </div>
                <Progress 
                  value={(estimation.totals.adjusted_points / (estimation.totals.adjusted_points + 10)) * 100} 
                  className="h-2"
                />
              </div>
              ...
              <div className="flex justify-end space-x-2">
                ...
                <Button 
                  ...
                  disabled={isApplying}
                  className={getLevelColor(selectedLevel).replace('bg-', 'bg-').replace('text-', 'text-white ')}
                >
                  {isApplying ? 'Applying...' : `Apply ${selectedLevel.charAt(0).toUpperCase() + selectedLevel.slice(1)} Estimation`}
                </Button>
              </div>
            </div>
          )}
          ...
          {isLoading && !estimation && (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground mt-2">Loading estimation...</p>
            </div>
          )}
        </CardContent>
      </Card>
